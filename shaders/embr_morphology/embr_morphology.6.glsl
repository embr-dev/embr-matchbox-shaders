...
void main(void)
{
    ...
    vec3 result = texture2D(adsk_results_pass5, uv).rgb;
    int r = 0;
    bool use_max = (size < 0);

    if (mode == 1 && size != 0 && (kernel == 0 || kernel == 2))
    {
        r = apply_strength(kernel_radius(clamp_radius(size), kernel, 0), uv);
        result = morph_1d(adsk_results_pass5, uv, step_from_angle(angle + 90.0), r, use_max, channel);
    }
