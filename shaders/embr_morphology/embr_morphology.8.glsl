...
void main(void)
{
    ...
    vec3 result = texture2D(adsk_results_pass7, uv).rgb;
    int r = 0;
    bool use_max = (size < 0);
    ...
    if (mode == 1 && size != 0 && kernel == 2)
    {
        ...
        result = morph_1d(adsk_results_pass7, uv, step_from_angle(angle + 135.0), r, use_max, channel);
    }

    result = mix(texture2D(front, uv).rgb, result, clamp(mix_amount, 0.0, 1.0));
